  ...
  const stopAudioRecording = async () => {
        ...
        mediaRecorderRef.current.onstop = () => {
          try {
            ...
            if (typeof window !== "undefined" && URL.createObjectURL) {
              ...
              console.log("Debug - Audio File:", file);
              
              // Optional: Create an audio element to play it back
              const audio = new Audio(audioUrl);
              console.log("Debug - Audio element:", audio);
            }
      ...
      try {
        ...
        setQHistory((prev) => [
          ...prev,
          {
            ...
            userAnswer:response?.data?.transcript,
    ...
    console.log("question histryo after dunction:",qHistory);
    ...
    const handleAgree = async () => {
      setOpenDialog(false);
      setLoadendViva(true);
      setCurrentQuestion('Succesfuly completed Viva!');
      // Save viva results to the database
      try {
        await axios.post(`${API}/viva/saveVivaResults`, {
          vivaId,
          username,
          qHistory, // All Gemini API responses
        });

        // toast.success("Viva results saved successfully!");
        navigate("/viva-end");
      } catch (error) {
        console.error("Error saving viva results:", error);
        // toast.error("Failed to save viva results");
      }
    };
